...
def get_relative_distance(agent_buying_ability, card):
    ...
    if distance > 0:
        distance -= agent_buying_ability['yellow']
...
class Action_Type():
    ...
    def __init__(self, agent, opponent, board, potential_nobles, good_card, agent_buying_ability, relative_distance):
        ...
        self.agent_gems = sum(self.agent.gems.values())
        self.opponent_card = []
    ...
    def calculate_reward(self, action):
        ...
        if action['type'] == 'buy_available' or  action['type'] == 'buy_reserve':
            ...
            reward += max(self.collection_reward, 32) if len(self.agent.cards['yellow']) < 3 else max(self.collection_reward, 20)
            ...
            reward += 2 if action['card'] in self.good_card else 0
            reward -= 20*sum(action['returned_gems'].values())
            current_reward = reward
            if action['card'] in self.opponent_card:
                    reward = max(self.collection_reward, self.buy_reward)
...
class MCTS():
    ...
    def get_appropriate_actions(self, game_state, agent_id):
        ...
        for card in board.dealt_list() + agent.cards['yellow']:
            if (card.colour in noble_color and len(agent.cards[card.colour]) < 4) or card.points > 1 or (card in agent.cards['yellow']): 
